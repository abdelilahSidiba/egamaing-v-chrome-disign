          ...
          children: [
            Row(
              children: [
                ...
                const Spacer(),
                Text('${rivalry.played} مباراة', style: const TextStyle(color: AppColors.textMuted, fontSize: 12)),
              ],
            ),
            const SizedBox(height: 8),
            Row(
  ...
  Widget build(BuildContext context) {
    switch (result) {
      ...
      case TournamentResult.participant:
        return const Text('مشارك', style: TextStyle(color: AppColors.textMuted));
    }
  }
}
