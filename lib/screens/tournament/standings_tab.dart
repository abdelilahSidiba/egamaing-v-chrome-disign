...
class _StandingsTable extends StatelessWidget {
  ...
  Color? _rowColor(int position, int total) {
    if (isSwissFormat) {
      if (position <= 8) return Colors.green.withValues(alpha: 0.12);
      if (position <= 24) return Colors.orange.withValues(alpha: 0.12);
      return Colors.red.withValues(alpha: 0.08);
    }
    if (qualifiersCount != null) {
      if (position <= qualifiersCount!) return Colors.green.withValues(alpha: 0.12);
    }
    if (position == 1) return AppColors.gold.withValues(alpha: 0.15);
    if (position == 2) return AppColors.textMuted.withValues(alpha: 0.15);
    if (position == 3) return const Color(0xFFCD7F32).withValues(alpha: 0.12);
    return null;
  }
