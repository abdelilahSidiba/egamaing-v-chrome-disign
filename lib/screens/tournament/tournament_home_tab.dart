...
class TournamentHomeTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final nextMatches = provider.matches
        .where((m) => m.status == MatchStatus.notPlayed && !m.isBye)
        .toList();
    nextMatches.sort((a, b) => a.round.compareTo(b.round));
    ...
    return ListView(
      ...
      children: [
        _IdentityCard(color: color, tournament: tournament),
        const SizedBox(height: 16),
        if (nextMatches.isNotEmpty)
          _NextMatchCard(match: nextMatches.first, provider: provider),
        const SizedBox(height: 16),
        _QuickStatsCard(
          stats: generalStats,
          remaining: provider.matches.where((m) => m.status == MatchStatus.notPlayed).length,
        ),
      ],
    );
  }
...
class _IdentityCard extends StatelessWidget {
  ...
  const _IdentityCard({required this.color, required this.tournament});
  ...
  @override
  Widget build(BuildContext context) {
    final progressPercent = tournament.progressPercent;
    return Container(
...
class _NextMatchCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final Team? home = provider.teamById(match.homeTeamId);
    final Team? away = provider.teamById(match.awayTeamId);
